...
{
  if(i < 0 || i > 1){
      std::cerr << "2D vector index bounds error" << std::endl;
    exit(1);
  }
...
{
  if(i < 0 || i > 2){
      std::cerr << "3D vector index bounds error" << std::endl;
    exit(1);
  }
...
{
  if(i < 0 || i > 3){
    std::cerr << "4D vector index bounds error" << std::endl;
    exit(1);
  }
...
{
  if(i < 0 || i >= N){
    std::cerr << N << " vector index bounds error" << std::endl;
    exit(1);
  }
...
void Vector::setsize(int vN){
  if(vN < 0){
    std::cerr << "vector dimension of " << vN << " invalid" << std::endl;
    exit(1);    
  }
  ...
  if(N == 0)
    ...
    v = new double[N];
    if(!v){
      std::cerr << "not enough memory to allocate vector of size " << N << std::endl;
      exit(1);    
    }
  }
...
Vector::operator Vector2d(){
  if(N > 2){
    std::cerr << "cannot convert " << N << " Vector to Vector2d" << std::endl;
    exit(1);    
  }
...
Vector::operator Vector3d(){
  if(N > 3){
    std::cerr << "cannot convert " << N << " Vector to Vector3d" << std::endl;
    exit(1);    
  }
...
Vector::operator Vector4d(){
  if(N > 4){
    std::cerr << "cannot convert " << N << " Vector to Vector4d" << std::endl;
    exit(1);    
  }
...
Vector2d Vector2d::normalize() const
{
  ...
  Vector2d newv;
  magnitude = norm();
  if (std::abs(x) > magnitude * HUGENUMBER ||
      std::abs(y) > magnitude * HUGENUMBER ){
    std::cerr << "Attempting to take the norm of a zero 2D vector." << std::endl;
  }
  newv.x = x / magnitude;
  newv.y = y / magnitude;
  return newv;
...
Vector3d Vector3d::normalize() const
{
  ...
  if (std::abs(x) > magnitude * HUGENUMBER ||
      std::abs(y) > magnitude * HUGENUMBER ||
      std::abs(z) > magnitude * HUGENUMBER ){
    std::cerr << "Attempting to take the norm of a zero 3D vector." << std::endl;
  }
  newv.x = x / magnitude;
  newv.y = y / magnitude;
  newv.z = z / magnitude;
...
Vector4d Vector4d::normalize() const
{
  ...
  if (std::abs(x) > magnitude * HUGENUMBER ||
      std::abs(y) > magnitude * HUGENUMBER ||
      std::abs(z) > magnitude * HUGENUMBER ){
    std::cerr << "taking the norm of a zero 4D vector." << std::endl;
  }
  newv.x = x / magnitude;
  newv.y = y / magnitude;
  newv.z = z / magnitude;
...
Vector4d Vector4d::wnorm() const
{
  ...
  if (w == 1.0)
    ...
    std::cerr << "w-Normalizing vector [" <<
      x << " " << y << " " << z<< " " << w << "] w ~= 0!" << std::endl;
...
void Vector::set(double vx, double vy){
  if(N < 2){
    std::cerr << "too many arguments for " << N << " Vector set" << std::endl;
    exit(1);
  }
...
{
  std::cout << "[" << std::setw(w) << std::setprecision(p) << Round(x, p) << " ";
  std::cout << std::setw(w) << std::setprecision(p) << Round(y, p) << "]";
}
...
void Vector3d::print(int w, int p) const
{
  std::cout << "[" << std::setw(w) << std::setprecision(p) << Round(x, p) << " ";
  std::cout << std::setw(w) << std::setprecision(p) << Round(y, p) << " ";
  std::cout << std::setw(w) << std::setprecision(p) << Round(z, p) << "]";
}
...
void Vector4d::print(int w, int p) const
{
  ...
  std::cout << std::setw(w) << std::setprecision(p) << Round(y, p) << " ";
  std::cout << std::setw(w) << std::setprecision(p) << Round(z, p) << " ";
  std::cout << std::setw(w) << std::setprecision(p) << Round(w, p) << "]";
}
...
void Vector::print(int w, int p) const
{
  if(N == 0)
    ...
    int i;
    std::cout << "[" << std::setw(w) << std::setprecision(p) << Round(v[0], p);
    for(i = 1; i < N; i++)
      std::cout << " " << std::setw(w) << std::setprecision(p) << Round(v[i], p);
    std::cout << "]";
  }
}
...
{
  if(v1.N != v2.N){
    std::cerr << "cannot add " << v1.N << " Vector to " << v2.N << " Vector" <<
      std::endl;
    exit(1);
  }
  Vector r(v1.N);
...
{
  if(v1.N != v2.N){
    std::cerr << "cannot subtract " << v2.N << " Vector from " << v1.N <<
      " Vector" << std::endl;
    exit(1);
  }
...
{
  if(v1.N != v2.N){
    std::cerr << "cannot take dot product of " << v1.N << " Vector with " <<
      v2.N << " Vector" << std::endl;
    exit(1);
  }
...
Vector4d operator%(const Vector4d& v1, const Vector4d& v2)
{
  std::cerr << "sorry, cross product of Vector4d's not yet implemented" << std::endl;
  exit(1);
...
{
  if(v1.N != v2.N || v1.N < 3){
    std::cerr << "cannot take cross product of " << v1.N << " Vector with " <<
      v2.N << " Vector" << std::endl;
    exit(1);
  }

  std::cerr << "sorry, (Vector % Vector) not implemented yet" << std::endl;
  exit(1);
  return v1;
}
